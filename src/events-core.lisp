    ...
    (cond
      ...
      ((= btn +mouse-btn-motion+)
       (if *mouse-drag-state*
           ...
           (destructuring-bind (split orient _ignored-col _ignored-row) *mouse-drag-state*
             (declare (ignore _ignored-col _ignored-row))
...
(defun make-overlay-escape-k (buffer)
  ...
  ;; BUFFER is captured from the enclosing call; we extend it byte by byte.
  (lambda (_ignored-session byte)
    ...
    (let ((length (fill-pointer buffer)))
      (cond
        ...
        ((and (= length 2) (= byte +byte-csi-bracket+))
