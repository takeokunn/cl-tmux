                ...
                (multiple-value-bind (align rest) (%split-align-attr (subseq raw (+ i 2) close))
                  (cond
                    (align (setf current align)
                           (when rest (format (%status-bucket-stream buckets current) "#[~A]" rest)))
                    (t (write-string (subseq raw i (1+ close))
                                     (%status-bucket-stream buckets current))))
                  (setf i (1+ close)))
                (progn (write-char (char raw i) (%status-bucket-stream buckets current))
                       (incf i))))
          (progn (write-char (char raw i) (%status-bucket-stream buckets current))
                 (incf i))))
