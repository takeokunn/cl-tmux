...
(defun run-kill-server (raw-args)
  "Send kill-server command via the socket, then exit.
   When no server can be reached, report the same connection failure shape as
   tmux's non-interactive command path."
  (let ((server-name (%running-server-name)))
    (unless (%forward-startup-command server-name "kill-server" raw-args)
      (format *error-output*
              "error connecting to ~A (No such file or directory)~%"
...
(defun run-list-windows (raw-args)
  "Print a list of windows to stdout and exit.
   Forwards to a running server so command output comes from server state."
  (unless (%forward-startup-command (%running-server-name) "list-windows" raw-args)
    (format *error-output*
            "error connecting to ~A (No such file or directory)~%"
            (socket-path "0"))
    (sb-ext:exit :code 1))
...
(defun run-display-message (raw-args)
  "Forward display-message to the running server and exit.
   Like tmux, this command requires a server even for `display -p ...`; the
   server-side dispatcher owns format context, hooks, and output capture."
  (let ((server-name (%running-server-name)))
    (unless (%forward-startup-command server-name "display-message" raw-args)
      (format *error-output*
              "error connecting to ~A (No such file or directory)~%"
              (socket-path (or server-name "0")))
      (sb-ext:exit :code 1)))
...
(defun run-show-options (raw-args)
  "Forward show-options to the running server and exit.
   Like tmux, non-interactive show-options requires a server; the server-side
   dispatcher owns scope selection, formatting, and option output."
  (let ((server-name (%running-server-name)))
    (unless (%forward-startup-command server-name "show-options" raw-args)
      (format *error-output*
              "error connecting to ~A (No such file or directory)~%"
              (socket-path (or server-name "0")))
      (sb-ext:exit :code 1)))
...
(defun run-show-window-options (raw-args)
  "Forward show-window-options to the running server and exit.
   Like tmux, non-interactive show-window-options requires a server; the
   server-side dispatcher owns target-window selection, flags, and output."
  (let ((server-name (%running-server-name)))
    (unless (%forward-startup-command server-name "show-window-options" raw-args)
      (format *error-output*
              "error connecting to ~A (No such file or directory)~%"
              (socket-path (or server-name "0")))
      (sb-ext:exit :code 1)))
