...
(defmacro define-erase-display-rules (&rest specs)
  "Build ERASE-DISPLAY from a Prolog-like mode rule table.
   ...
   CX, CY, W, H, SCREEN are available via a WITH-SCREEN-COORDS helper;
   each arm uses a local let for only the variables it needs, so no
   global (declare (ignorable ...)) is required."
...
(define-erase-display-rules
  (0
   (let ((cx (screen-cursor-x screen)) (cy (screen-cursor-y screen))
         (w  (screen-width    screen)) (h  (screen-height   screen)))
...
(defmacro define-erase-line-rules (&rest specs)
  "Build ERASE-LINE from a declarative range table.
   ...
   All three variables are referenced across the rule arms so no ignorable is needed."
