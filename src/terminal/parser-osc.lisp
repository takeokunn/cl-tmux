  ...
  (let* ((alphabet "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")
    ...
    (handler-case
        (loop for group-start from 0 below input-length by 4
              do (let* ((index0 (%base64-char-index alphabet (char encoded-string group-start)))
                        ...
                        ;; Reconstruct the three output bytes from the four 6-bit indices.
...
;;;
;;; Design note: parse-integer failures are caught individually so that a
;;; malformed command number only suppresses that OSC command — it does not
;;; swallow subsequent OSC sequences silently.
...
(defun make-osc-k (buf)
  "Return a continuation that accumulates OSC payload bytes into BUF.
