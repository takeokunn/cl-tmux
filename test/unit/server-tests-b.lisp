...
(test parse-attach-flags-table
  ...
  (dolist (row '((()                                       "0"             nil nil "no args → default name \"0\"")
                 ...
                 (("-t" "combo" "-d" "-r")                 "combo"         t   t   "all flags combined")
                 (("--unknown" "-t" "after-unknown" "-d")  "after-unknown" t   nil "unknown flag is skipped")))
    (destructuring-bind (args expected-name expected-detach expected-ro desc) row
      (multiple-value-bind (name detach ro) (cl-tmux::%parse-attach-flags args)
        (is (string= expected-name name)              "~A: name"   desc)
        (is (eq expected-detach (if detach t nil))    "~A: detach" desc)
        (is (eq expected-ro    (if ro    t nil))      "~A: ro"     desc)))))
