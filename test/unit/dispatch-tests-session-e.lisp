...
(test named-command-break-pane-is-recognized
  ...
  (with-fake-session (s :nwindows 1 :npanes 2)
    (with-dispatch-overlay (s "break-pane")
      (assert-overlay-not-contains "unknown command" *overlay*
...
(test named-command-unknown-shows-error-overlay
  ...
  (with-fake-session (s)
    (with-dispatch-overlay (s "no-such-command-xyz")
    ...
    (let ((s2 (make-fake-session :nwindows 1 :npanes 1)))
      (with-session-name (s1 "alpha")
        (with-session-name (s2 "beta")
          (with-registered-sessions ((session-name s1) (session-name s2))
            (with-run-command-line-overlay
