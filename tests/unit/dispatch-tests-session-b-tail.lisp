...
(test default-size-dimensions-derive-from-size-string
  "%default-size-dimensions returns the detached-session size from the
   default-size option, and NIL when the session is attached."
  (with-isolated-options ("default-size" "100x40")
    (multiple-value-bind (cols rows)
        (cl-tmux::%default-size-dimensions t)
      (is (= 100 cols) "default-size width becomes detached-session columns")
      (is (= 40 rows) "default-size height becomes detached-session rows"))
    (is (null (cl-tmux::%default-size-dimensions nil))
        "attached sessions ignore default-size")))
