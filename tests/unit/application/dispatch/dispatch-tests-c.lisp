...
(test run-command-line-list-keys-dash-1-keeps-only-first-line
    ...
    (let ((*overlay* nil))
      ...
      (let ((full *overlay*))
        ...
        (let* ((newline (position #\Newline full))
          ...
          (is (string= *overlay* first-line)
              "dash-one output must match the first line of full output")
          (is (null (position #\Newline *overlay*))
              "dash-one output must be single line"))))))
