...
(test copy-mode-row-blank-p-distinguishes-blank-and-content-rows
  "%copy-mode-row-blank-p returns T only for fully blank rows."
  (let ((s (copy-mode-screen :w 8 :h 3 :content "ab")))
    (is-false (cl-tmux/commands::%copy-mode-row-blank-p s 0)
              "row 0 contains text and must not be considered blank")
    (is-true (cl-tmux/commands::%copy-mode-row-blank-p s 1)
             "row 1 is untouched and must be considered blank")))

(test find-paragraph-boundary-scans-to-nearest-blank-row
  "%find-paragraph-boundary returns the nearest blank row or the edge clamp."
  (let ((s (make-screen 20 5)))
    (feed-lines s "hello" "world" "" "foo" "bar")
    (is (= 2 (cl-tmux/commands::%find-paragraph-boundary s 1 :down 5))
        "scan down from row 1 must stop at blank row 2")
    (is (= 2 (cl-tmux/commands::%find-paragraph-boundary s 3 :up 0))
        "scan up from row 3 must stop at blank row 2")
    (is (= 4 (cl-tmux/commands::%find-paragraph-boundary s 4 :down 5))
        "scan down with no blank below must clamp to the last row")))
