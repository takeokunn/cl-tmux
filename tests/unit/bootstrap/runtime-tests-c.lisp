...
(test add-message-log-mirrors-to-current-client-log
  :description "add-message-log also appends to the current client's message log."
  (let ((cl-tmux::*message-log* nil)
        (cl-tmux::*current-client-conn* (cl-tmux::%make-client-conn
                                         :state (cl-tmux::make-input-state))))
    (cl-tmux::add-message-log "client-scoped")
    (is (string= "client-scoped" (cdr (first cl-tmux::*message-log*)))
        "global message log must receive the message")
    (is (string= "client-scoped"
                 (cdr (first (cl-tmux::client-conn-message-log
                              cl-tmux::*current-client-conn*))))
        "current client message log must receive the message")))
