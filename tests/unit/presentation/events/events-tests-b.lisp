...
(describe "events-suite"
  ...
  (it "sgr-mouse-sequence-p-returns-nil-for-short-buffer"
    (flet ((mk-buf (s)
  ...
  (it "apply-drag-resize-horizontal-updates-ratio"
  ...
  (it "dispatch-modifier-arrow-ctrl-arrow-resizes-one-cell"
    (with-fake-session (s)
    ...
    (with-fake-session (s)
      (let ((screen (active-screen s)))
        ...
        (let* ((cursor (screen-copy-cursor screen))
               (row    (car cursor))
               (col    (cdr cursor)))
          (expect (<= 0 row (1- (screen-height screen))))
          (expect (<= 0 col (1- (screen-width screen))))))))
  ...
  (it "copy-mode-j-at-interior-row-moves-cursor-not-scrolls"
  ...
  ;; scrolling the viewport.  The offset must remain unchanged.
  (it "copy-mode-k-at-interior-row-moves-cursor-not-scrolls"
    (with-copy-mode-vi-state (s screen state)
      (seed-scrollback screen 10)
  ...
  (it "border-at-position-detects-h-split-border"
    (let* ((p0    (make-pane :id 1 :fd -1 :pid -1 :x 0 :y 0 :width 40 :height 24
                              :screen (make-screen 40 24)))
           (p1    (make-pane :id 2 :fd -1 :pid -1 :x 41 :y 0 :width 40 :height 24
                              :screen (make-screen 40 24)))
  ...
  (it "border-at-position-returns-nil-inside-pane"
    (let* ((p0    (make-pane :id 1 :fd -1 :pid -1 :x 0 :y 0 :width 40 :height 24
                              :screen (make-screen 40 24)))
           (p1    (make-pane :id 2 :fd -1 :pid -1 :x 41 :y 0 :width 40 :height 24
                              :screen (make-screen 40 24)))
  ...
  (it "border-at-position-returns-nil-for-single-pane-window"
  ...
  (it "mouse-status-bar-click-does-nothing-for-out-of-range-col"
  ...
  (it "copy-mode-dollar-moves-to-line-end"
    (with-copy-mode-vi-state (s screen state)
      (feed screen "abc")
      ;; Start at col 0.
  ...
  (it "copy-mode-ctrl-p-scrolls-up"
    (with-copy-mode-emacs-state (s screen state)
      (seed-scrollback screen 10)
