...
(test cmd-respawn-pane-forwards-overrides
  ...
  (with-fake-session (s :nwindows 1 :npanes 1)
    (let* ((pane (window-active-pane (session-active-window s)))
           ...
           (cl-tmux::*dirty* nil)
           (*overlay* nil)
           (orig-respawn-pane (fdefinition 'cl-tmux::respawn-pane))
           (orig-start-reader-thread (fdefinition 'cl-tmux::start-reader-thread)))
      (unwind-protect
           (progn
             (setf (fdefinition 'cl-tmux::respawn-pane)
                   (lambda (session pane &key start-dir default-command extra-env)
                     (push (list session pane start-dir default-command extra-env) calls)
                     pane))
             (setf (fdefinition 'cl-tmux::start-reader-thread)
                   (lambda (pane)
                     (push pane reader-calls)
                     nil))
        ...
        (setf (fdefinition 'cl-tmux::respawn-pane) orig-respawn-pane
              (fdefinition 'cl-tmux::start-reader-thread) orig-start-reader-thread)))))
