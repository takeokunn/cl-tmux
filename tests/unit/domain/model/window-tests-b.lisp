...
;;; using make-no-pty-pane from helpers.lisp and make-fake-window from helpers-b.lisp.
...
(test rotate-window-while-zoomed-preserves-zoom-state
  ...
  (let* ((p0  (make-no-pty-pane 1 0 0 20 5))
    ...
    (is-true (cl-tmux/model::window-zoom-p win)
             "rotate must keep the window zoomed")
    (is (equal (list p0) (window-panes win))
        "visible zoomed panes list must stay on the active pane")
    (cl-tmux/model:window-zoom-toggle win)
    (is (equal (list p1 p2 p0) (window-panes win))
        "unzooming after rotate must restore the rotated pane order")))
