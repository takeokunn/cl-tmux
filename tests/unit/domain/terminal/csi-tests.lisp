...
(def-suite decrqm
  :description "DECRQM mode query replies — enqueue-decrqm-reply"
  :in terminal-suite)
(in-suite decrqm)

(test decrqm-accessor-mode-reports-set-and-reset
  ...
  (with-screen (s 20 5)
    ...
    (let ((reply (first (cl-tmux/terminal/types:screen-response-queue s))))
      (is (string= (format nil "~C[?1004;2$y" #\Escape) reply)
          "mode 1004 unset must report ;2$y (got ~S)" reply))
    ...
    (cl-tmux/terminal/csi::enqueue-decrqm-reply s 1004)
    (let ((reply (first (cl-tmux/terminal/types:screen-response-queue s))))
      (is (string= (format nil "~C[?1004;1$y" #\Escape) reply)
          "mode 1004 set must report ;1$y (got ~S)" reply))))

(test decrqm-fixed-mode-always-reports-given-code
  ...
  (with-screen (s 20 5)
    ...
    (let ((reply (first (cl-tmux/terminal/types:screen-response-queue s))))
      (is (string= (format nil "~C[?2026;2$y" #\Escape) reply)
          "mode 2026 (:fixed 2) must always report ;2$y (got ~S)" reply))))

(test decrqm-mouse-mode-reports-active-and-inactive
  ...
  (with-screen (s 20 5)
    ...
    (is (string= (format nil "~C[?1000;2$y" #\Escape)
                 (first (cl-tmux/terminal/types:screen-response-queue s)))
        "mode 1000 with mouse-mode=0 must report ;2$y")
    ;; activate mouse mode 1: mode 1000 -> code 1
    (setf (cl-tmux/terminal/types:screen-response-queue s) nil)
    (setf (cl-tmux/terminal/types:screen-mouse-mode s) 1)
    (cl-tmux/terminal/csi::enqueue-decrqm-reply s 1000)
    (is (string= (format nil "~C[?1000;1$y" #\Escape)
                 (first (cl-tmux/terminal/types:screen-response-queue s)))
        "mode 1000 with mouse-mode=1 must report ;1$y")))

(test decrqm-alt-screen-mode-reflects-alternate-screen-state
  ...
  (with-screen (s 20 5)
    ...
    (is (string= (format nil "~C[?1049;2$y" #\Escape)
                 (first (cl-tmux/terminal/types:screen-response-queue s)))
        "mode 1049 before switching to alt-screen must report ;2$y")
    ;; Switch to alt screen via ESC[?1049h
    (setf (cl-tmux/terminal/types:screen-response-queue s) nil)
    (feed s (esc "[?1049h"))
    (cl-tmux/terminal/csi::enqueue-decrqm-reply s 1049)
    (is (string= (format nil "~C[?1049;1$y" #\Escape)
                 (first (cl-tmux/terminal/types:screen-response-queue s)))
        "mode 1049 with alt-screen active must report ;1$y")))
